...
def search_by_title(title):
    ...
    try:
        ...
        # 결과가 없으면 두 번째 시도: 따옴표 없이 검색
        if not pmids:
            st.info("⚠️ 정확한 제목으로 검색 실패, 유연한 검색 시도 중...")
            params['term'] = f'{title}[Title]'
            st.info(f"🔍 검색 시도 2 (유연한 검색): {params['term']}")
            response = requests.get(url, params=params, verify=False)
            response.raise_for_status()
            data = response.json()
            pmids = data['esearchresult']['idlist']
        
        # 여전히 결과가 없으면 세 번째 시도: Title/Abstract 검색
        if not pmids:
            st.info("⚠️ Title 검색 실패, Title/Abstract 검색 시도 중...")
            params['term'] = f'{title}[Title/Abstract]'
            st.info(f"🔍 검색 시도 3 (Title/Abstract): {params['term']}")
            response = requests.get(url, params=params, verify=False)
            response.raise_for_status()
            data = response.json()
            pmids = data['esearchresult']['idlist']
...
with col1:
    search_type = st.radio("검색 방식", ["Title", "DOI"], horizontal=True)
